...
public class AtUnit implements ProcessFiles.Strategy {
    ...
    static class TestMethods extends ArrayList<Method>{
        void addIfTestMethod(Method m){
            ...
            if(!(m.getReturnType().equals(testClass)||m.getReturnType().equals(void.class))){
                throw new RuntimeException("@Test method must be return boolean or void");
    ...
    private static Method checkForCleanupMethod(Method m){
        ...
        if(m.getParameterTypes().length==0||m.getParameterTypes()[0]!=testClass){
            throw new RuntimeException("@TestObjectCleanup must take an argument of the testd type.");
        }
        m.setAccessible(true);
        return m;
    }
